...
class GameState {
    ...
    bindCanvasEvents() {
        ...
        if (importCanvasBtn && importCanvasFile) {
            importCanvasBtn.addEventListener('click', () => {
                importCanvasFile.click();
            });
            
            importCanvasFile.addEventListener('change', async (e) => {
                const file = e.target.files[0];
                if (file) {
                    await this.importCanvasData(file);
                    // 清空文件选择器，允许重复选择同一文件
                    e.target.value = '';
                }
            });
    ...
    exportCanvasData() {
        ...
        link.download = `煎饼摊画布_房间${this.currentRoom}_${timestamp}.json`;
        ...
        this.showToast('画布数据已导出', 'success');
    }

    // 导入画布数据从JSON文件
    async importCanvasData(file) {
        if (!this.currentRoom) {
            this.showToast('请先加入房间', 'error');
            return;
        }
        ...
        try {
            ...
            // 验证数据格式
            if (!importData.poems || !importData.grid) {
                this.showToast('文件格式不正确', 'error');
                return;
            }

            // 确认导入操作
            if (!confirm(`确定要导入画布数据吗？\n这将覆盖当前房间的所有内容！\n\n导出时间：${importData.exportTime || '未知'}\n诗句数量：${importData.poems.length}`)) {
                return;
            }

            // 发送到后端进行导入
            const response = await fetch(`/api/canvas/import/${this.currentRoom}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    poems: importData.poems,
            ...
            if (result.success) {
                // 更新本地数据
                this.poems = importData.poems;
                this.grid = importData.grid;
                this.renderCanvas();
                this.updateUI();
                
                this.showToast('画布数据导入成功', 'success');
                
                // 通知其他用户刷新
                if (this.socket) {
                    this.socket.emit('canvas_imported', {
    ...
    initSocket() {
        ...
        this.socket.on('canvas_imported', async (data) => {
            this.showToast(`${data.imported_by} 导入了画布数据（${data.poem_count}首诗句）`, 'info');
            // 重新加载房间数据
...
class JianbingGame {
    ...
    bindEvents() {
        ...
        const supportImageWrapper = document.getElementById('supportImageWrapper');
        ...
        // 展开/收起赞助图片
        if (toggleSponsor && supportImageWrapper) {
            toggleSponsor.addEventListener('click', (e) => {
                e.preventDefault();
                const isShown = supportImageWrapper.style.display !== 'none';
                supportImageWrapper.style.display = isShown ? 'none' : 'block';
            });
        }
...
function initFloatingMenu() {
    ...
    const sharedSpaceBtn = document.getElementById('sharedSpaceBtn');
    
    if (!floatingBtn || !sidebar) return;
    ...
    // 点击共享空间按钮
    if (sharedSpaceBtn) {
        sharedSpaceBtn.addEventListener('click', () => {
            window.location.href = '/Sharedroom';
        });
    }
    
    // 点击AI煎饼摊按钮
    const aiBingBtn = document.getElementById('aiBingBtn');
    if (aiBingBtn) {
        aiBingBtn.addEventListener('click', () => {
            window.open('http://bing.v50tome.cn/', '_blank');
        });
    }
    ...
    const downloadMainApp = document.getElementById('downloadMainApp');
    const downloadAIApp = document.getElementById('downloadAIApp');
    
    if (mobileDownloadBtn && downloadSubmenu) {
        // 点击展开/收起子菜单
        mobileDownloadBtn.addEventListener('click', () => {
            const isExpanded = mobileDownloadBtn.classList.contains('expanded');
            
            if (isExpanded) {
                mobileDownloadBtn.classList.remove('expanded');
                downloadSubmenu.style.display = 'none';
            } else {
                mobileDownloadBtn.classList.add('expanded');
    ...
    // 下载主应用
    if (downloadMainApp) {
        downloadMainApp.addEventListener('click', () => {
            downloadAPK('煎饼摊.apk');
        });
    }
    ...
    // 下载AI应用
    if (downloadAIApp) {
        downloadAIApp.addEventListener('click', () => {
            downloadAPK('AI煎饼摊.apk');
        });
    }
}
...
function downloadAPK(filename) {
    try {
        // 创建下载链接
        const link = document.createElement('a');
        link.href = `/download/apk/${encodeURIComponent(filename)}`;
        link.download = filename;
        ...
        if (window.game && window.game.gameState) {
            window.game.gameState.showToast(`开始下载 ${filename}`, 'success');
        } else {
            alert(`开始下载 ${filename}`);
        }
        
        console.log(`下载APK: ${filename}`);
    } catch (error) {
        console.error('下载失败:', error);
        if (window.game && window.game.gameState) {
            window.game.gameState.showToast('下载失败，请稍后重试', 'error');
        } else {
            alert('下载失败，请稍后重试');
        }
    }
...
// 检查并更新固定房间UI
function checkAndUpdateFixedRoomUI() {
    ...
    // 固定房间列表
    const fixedRooms = ['100001', '100002', '100003', '100004', '100005', '100006'];
    
    if (fixedRooms.includes(currentRoom)) {
        // 这是固定房间，禁用重置按钮（除非是管理员）
        const resetBtn = document.getElementById('resetBtn');
        const username = game.gameState.currentUser;
        
        if (resetBtn && username !== '管理员') {
            resetBtn.disabled = true;
            resetBtn.title = '固定房间只能由管理员重置';
            ...
            resetBtn.addEventListener('click', (e) => {
                if (resetBtn.disabled) {
                    e.preventDefault();
                    e.stopPropagation();
                    game.gameState.showToast('固定房间只能由管理员重置', 'warning');
                }
            });
        }
        ...
        if (roomCodeDisplay && !roomCodeDisplay.querySelector('.fixed-room-badge')) {
            const badge = document.createElement('span');
            badge.className = 'fixed-room-badge';
            badge.textContent = '🏛️固定';
            badge.style.cssText = 'margin-left: 8px; padding: 2px 6px; background: #8b7355; color: white; border-radius: 4px; font-size: 11px;';
            roomCodeDisplay.appendChild(badge);
        }
    }
}
